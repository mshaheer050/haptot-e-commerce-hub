...
const ProductSkeletonRow = () => (
  <div className="container mx-auto py-8">
    <div className="h-6 w-40 bg-muted rounded-2xl mb-4 animate-pulse" />
    <div className="flex gap-4 overflow-hidden">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="min-w-[200px] rounded-3xl bg-muted animate-pulse h-64" />
      ))}
    </div>
  </div>
);
...
const Index = () => {
  ...
  return (
    <main>
        ...
        <div className="container mx-auto">
          <div className="grid grid-cols-3 gap-4">
            {trustBadges.map((b) => (
              <div
                key={b.label}
                className="flex flex-col items-center text-center p-4 md:p-6 rounded-3xl bg-card shadow-soft"
              >
                <b.icon className="w-6 h-6 text-primary mb-2" />
                <span className="font-display font-bold text-sm text-foreground">{b.label}</span>
      ...
      {/* Featured products — skeleton while loading, products when ready */}
      {loadingFeatured ? (
        <ProductSkeletonRow />
      ) : featured.length > 0 ? (
        <ProductScroller
          title="Trending Now"
          subtitle="Most loved by parents this week"
          products={featured}
        />
      ) : null}
        ...
        <div className="container mx-auto">
          <div className="bg-gradient-to-br from-pastel-blue to-pastel-orange rounded-3xl p-8 md:p-16 text-center">
            ...
            <p className="text-muted-foreground mt-3 max-w-md mx-auto">
              Get updates on new arrivals, exclusive offers, and parenting tips.
            </p>
            <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto mt-8">
              <input
                type="email"
                placeholder="your@email.com"
                className="flex-1 px-5 py-3 rounded-2xl bg-card border border-border text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
              />
